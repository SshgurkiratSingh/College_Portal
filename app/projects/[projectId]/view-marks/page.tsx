...
const ViewMarksPage = () => {
  ...
  const fetchStudentScores = async () => {
    try {
      ...
      const allScores = await apiClient.get<Score[]>(
        `/api/projects/${projectId}/scores/all`
      );
      ...
      if (studentScores.length === 0) {
        setError(
          "No scores found for this project. Your instructor may not have graded your work yet."
        );
      }
      ...
      const sortedScores = studentScores.sort((a, b) => {
        const numA =
          typeof a.questionNum === "number"
            ? a.questionNum
            : parseInt(a.questionNum.toString());
        const numB =
          typeof b.questionNum === "number"
            ? b.questionNum
            : parseInt(b.questionNum.toString());
        return numA - numB;
      });
  ...
  const getPassFailStatus = () => {
    ...
    return (
      <div
        className={`mt-2 p-2 text-center font-bold rounded ${
          passed ? "bg-green-800 text-white" : "bg-red-800 text-white"
        }`}
      >
  ...
  if (loading) {
    return (
      <Container>
        <div className="pt-24">
          <Heading title="Loading your marks..." subtitle="" />
      ...
      <Container>
        <div className="pt-24">
          <EmptyState
            ...
            subtitle="The project you're looking for does not exist or you don't have access to it."
          />
        </div>
      </Container>
    );
  ...
  return (
    <Container>
      <div className="pt-24">
        <Heading
          title={project.name}
          subtitle={project.description || "View your marks for this project"}
        />
              ...
              <div className="flex flex-wrap justify-between">
                <div className="mb-4">
                  ...
                  <p className="text-gray-300">
                    Percentage:{" "}
                    {totalMaxMarks > 0
                      ? ((totalScore / totalMaxMarks) * 100).toFixed(2)
                      : 0}
                    %
                  </p>
                </div>
                ...
                {project.passingMarks && (
                  <div className="mb-4">
                    ...
                    <p className="text-gray-300 mt-2">
                      Passing marks: {project.passingMarks}
                    </p>
                  </div>
                )}
              </div>
